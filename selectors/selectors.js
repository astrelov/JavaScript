...
function getHTMLNodeFromTree(dataTree) {
  ...
  function pushToFragment(nestedSetMap) {
    nestedSetMap.forEach((childs, node) => {
      const op = new Option(node.name, node.id);
      setLeftPadding.call(op, node.depth * 2);
      fragment.appendChild(op);
      pushToFragment(childs);
    });
  }

  pushToFragment(dataTree);
  return fragment;
}
